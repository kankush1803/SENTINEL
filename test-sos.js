...
async function testSOS() {
  try {
    ...
    const incidentRes = await axios.post(`${BACKEND_URL}/incidents`, {
      type: 'FIRE',
      location: 'Main Kitchen, Floor 1',
      description: 'Grease fire reported in the main kitchen area. Multiple staff members evacuated.'
    }, {
    ...
    setTimeout(async () => {
      ...
      const latest = allIncidents.data.find(inc => inc.id === incidentRes.data.id);
